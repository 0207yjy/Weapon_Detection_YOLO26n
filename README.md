...
This repository contains a fine-tuned YOLOv26n model for weapon detection based on the SOHAS dataset.
...
| **Features** | NMS-free, End-to-End, CPU Optimized |
...
- `weights/best.onnx` — ONNX model for deployment
...
session = ort.InferenceSession("weights/best.onnx", providers=['CPUExecutionProvider'])
...
2. Copy model files:
```bash
scp weights/best.onnx pi@raspberry-ip:/home/pi/model/
```

3. Run inference:
```bash
python3 inference.py --image test.jpg --model best.onnx
```
...
- YOLOv26 is NMS-free and optimized for edge deployment.
